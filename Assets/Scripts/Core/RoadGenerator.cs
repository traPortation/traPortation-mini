...
namespace TraPortation.RoadGenerator
{
    public class Line
    {
    ...
    public class RoadGenerator
    {
        ...
        public void GenerateRoads()
        {
            while (this.roads.Count < 30)
            {
                ...
                var angle = Random.Range(0.0f, 2.0f) * Mathf.PI;
                var angle_rev = angle + Mathf.PI;

                int nearestIdx = -1;
                float nearestDist = float.MaxValue;
                Vector2 start, end;
                bool startIsEdge = false;

                for (int j = 0; j < roads.Count; j++)
                {
                    var road = roads[j];
                    var (collide, point) = road.CollideToHalfLine(p, angle);
                    if (collide)
                    {
                ...
                if (nearestIdx != -1)
                {
                    ...
                    if (Mathf.Abs(this.roads[nearestIdx].Angle() - angle) < 0.5)
                    {
                        continue;
                    }

                    start = roads[nearestIdx].CollideToHalfLine(p, angle).Item2;
                }
                else
                {
                    startIsEdge = true;
                ...
                {
                    var road = roads[j];
                    var (collide, point) = road.CollideToHalfLine(p, angle_rev);
                    if (collide)
                    {
                        var dist = (point - p).magnitude;
                        if (dist < nearestDist)
                ...
                if (nearestIdx != -1)
                {
                    ...
                    if (Mathf.Abs(this.roads[nearestIdx].Angle() - angle_rev) < 0.5)
                    {
                        continue;
                    }
                    end = roads[nearestIdx].CollideToHalfLine(p, angle_rev).Item2;
                }
                else
                {
                    if (startIsEdge && this.roads.Count != 0)
                    {
                        continue;
                    }
                    end = Line.EdgePoint(p, angle_rev);
                }

                this.roads.Add(new Line(start, end));
            }
        }
